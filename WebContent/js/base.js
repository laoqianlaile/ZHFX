...
$(document).ready(function(){
	...
	*/
	...
	/*数据展示 切换  by hanwy*/
	$('.totalInfo i,.totalInfo p').click(function(){
		$('.totalInfo').removeClass('active');
		$(this).parent('.totalInfo').addClass('active');
	});
...
function Tab(objA,objB,className){
	$(objB).addClass(className);
	$(objA).removeClass(className);
}
//隐藏显示
function showHide(objA,objB){
	$(objA).siblings('.chartHeight').addClass('clas22').css("visibility","hidden").hide();
	//$(objB).hide();
	$(objA).css("visibility","visible").show();
}
/*添加图表*/
