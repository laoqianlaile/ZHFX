    ...
    <script type="text/javascript">
    	...
    	window.onload = function(){
        	for (var i=0;i<5;i++) {
            	if (parentWin.win) {
            		//注册项目类型
                	parentWin.win["viewSituationIndustry"] = window;
            	}else{
            ...
            //单击更多触发事件
         	$('.chartDivTitle').find(".more").bind("click",function(){
         		var url = "${ctx}/view/overView!getAuditPreparationByCY?orderby="+orderby;
    	...
    	function loadData() {
    		...
    		// 异步加载项目审备核类型
            $.ajax({
                url: "${ctx}/view/overView!getAuditPreparationByCY",
                data : params,
                dataType: "json",
                success: function (data) {
                	...
                	if (data) {
                        ...
                        for (var i=0,len=data.length; i<len; i++) {
                        	//名称
                            var VipArea=data[i]["itemCode"];
                        	if (data[i]["itemCode"] == '1') {
                        		VipArea = '第一大产业';
                        	} else if (data[i]["itemCode"] == '2') {
                        		VipArea = '第二大产业';
                        	} else if (data[i]["itemCode"] == '3') {
                        		VipArea = '第三大产业';
                        	}
                            // 值大小
