...
<body>
	<div id="fhyhz" class="chartDiv divContent">
        ...
        <h6 class="chartDivTitle">
          ...
          <span id="titleTxt">审核备项目投资环比</span>
        	<i title="设为主控"></i>
<!--         	<a class="more" href="javaScript:void(0)">更多>></a> -->
    ...
    <script type="text/javascript">
        ...
        function  switchTitleTxt(name){
            switch (name){
                case "gs": $("#titleTxt").text("审核备项目个数环比"); break;
                case "tz": $("#titleTxt").text("审核备项目投资环比");break;
            }
        }
  		//定义父窗口
    	...
    	window.onload = function(){
        	for (var i=0;i<5;i++) {
            	if (parentWin.win) {
            		//将当前窗口定义到父级窗口
                	parentWin.win["viewMonthTrends"] = window;
            	}else{
  		...
  		var option = {
  			    ...
  			    legend: {
                    ...
                    selected: {
  			        ...
  			        data:['2014年(柱)','2015年(柱)','2016年(柱)','2014年','2015年','2016年']
  			    },
                grid:{
                    borderWidth:0,
                    x: 100,
    	...
    	function loadData() {
            ...
            $.ajax({
                ...
                success: function (data) {
                        ...
                        for (var i=0,len=data.length; i<len; i++) {
                        	...
                        	if (date.indexOf('2014') > -1) {
                        		if (i == 0) {
                        			...
                        			var trend = Number((Number(data[i]["investMon"]||0) - Number(data[i-1]["investMon"]||0)) * 100/Number(data[i-1]["investMon"]||1)).toFixed(2);
                        			var trendCnt = Number((Number(data[i]["cnt"]||0) - Number(data[i-1]["cnt"]||0)) * 100/Number(data[i-1]["cnt"]||1)).toFixed(2);
                        			invTrend.push(trend);
                        			cntTrend.push(trendCnt);
                        		}
                        		totalMon = parseInt(data[i]["investMon"]||0);
                   				totalCnt = parseInt(data[i]["cnt"]||0);
                        	...
                        	if (date.indexOf('2015') > -1) {
                        		if (i == 12) {
                                    ...
                                    var trendCnt1=0;
                        		    if( i>=1){
                                        trend1= Number((Number(data[i]["investMon"]||0) - Number(data[i-1]["investMon"]||0)) * 100/Number(data[i-1]["investMon"]||1)).toFixed(2);
                                        trendCnt1= Number((Number(data[i]["cnt"]||0) - Number(data[i-1]["cnt"]||0)) * 100/Number(data[i-1]["cnt"]||1)).toFixed(2);
                                    }
                        			invTrend1.push(trend1);
                        			cntTrend1.push(trendCnt1);
                        		}
                        		totalMon1 = parseInt(data[i]["investMon"]||0);
                   				totalCnt1 = parseInt(data[i]["cnt"]||0);
                        	...
                        	if (date.indexOf('2016') > -1) {
                        		if (i == 24) {
                        			...
                        			var trend2 = Number((Number(data[i]["investMon"]||0) - Number(data[i-1]["investMon"]||0)) * 100/Number(data[i-1]["investMon"]||1)).toFixed(2);
                        			var trendCnt3 = Number((Number(data[i]["cnt"]||0) - Number(data[i-1]["cnt"]||0)) * 100/Number(data[i-1]["cnt"]||1)).toFixed(2);
                        			invTrend2.push(trend2);
                        			cntTrend2.push(trendCnt3);
                        		}
                        		totalMon2 = parseInt(data[i]["investMon"]||0);
                   				totalCnt2 = parseInt(data[i]["cnt"]||0);
                    ...
                    option.series[5].data = invTrend2;
                    //总投资
                    ...
                    optionNum.series[5].data = cntTrend2;
                    //加载隐藏
                    myChart.hideLoading();
                    //渲染对象
