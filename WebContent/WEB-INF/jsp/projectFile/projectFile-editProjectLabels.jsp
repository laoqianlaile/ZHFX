...
<div class="body">
	<div data-options="region:'center'" style="overflow: float;">
		<div id="privateLabelsTreeView" class="ztree" style="float:left"></div>
...
<script type="text/javascript">
	...
	$.ajax({
		...
		dataType : "json",
		traditional : true,
		url : "${ctx}/projectFile/projectFile!getCustomLabelsData2",//请求的action路径  
		error : function() {//请求失败处理函数  
		// 		            alert('请求失败');  
		},
		success : function(data) { //请求成功后处理函数。    
			$.fn.zTree.init($("#privateLabelsTreeView"),
					privateLabelSetting, data);
			var zTree = $.fn.zTree.getZTreeObj("privateLabelsTreeView");
	 ...
	 */
	 function getLabelsByProjectID() {
		$.ajax({
			...
			dataType : "json",
			traditional : true,
			url : "${ctx}/projectFile/projectFile!getPrivateLabelsByProjectID?projectId="+projectID+"&moduleCode="+moduleCode,//请求的action路径  
			error : function() {//请求失败处理函数  
			// 	            alert('请求失败');  
			},
			success : function(data) { //请求成功后处理函数。    
				...
				var zTree = $.fn.zTree
						.getZTreeObj("privateLabelsTreeView");
				if (data != null && data.length > 0) {
					var nodes = zTree.getNodes();
					// 循环设置选中节点
					setMuiltesCheckNodes(nodes, "privateLabelsTreeView", data);
				}
			}
		});
	};
	 ...
	 */
	 function updateProjectLabels(){
		var newProjectLabels = $.fn.zTree.getZTreeObj("privateLabelsTreeView").getCheckedNodes(true);
		var labelIDs = '';
		...
		var responseText = $.ajax({
			async : false,
			cache : false,
			data : {"labelParams.labelIDs":labelIDs,
					"labelParams.projectID":projectID	
			},
			type : 'POST',
			dataType : 'text',
			url : '${ctx}/projectFile/projectFile!updateProjectLabels.action',
			error : function() {//请求失败处理函数  
