var w = c.width = window.innerWidth,
    ...
    opts = {
        ...
        ellipseAxisMultiplierX: 1.8,
        ellipseAxisMultiplierY: .8,
        ellipseCX: w / 2,
        ellipseCY: 7 * h / 13,
...
function draw() {
    ...
    ctx.fillStyle = 'rgba(4,13,44,alp)'.replace('alp', opts.repaintAlpha);
    ctx.fillRect(0, 0, w, h);
...
Line.prototype.draw = function () {
    ...
    hc = tick + this.x / ( w * ( opts.ellipseBaseRadius + opts.ellipseAddedRadius ) ) * 100;
    var l = 75 - ratio * 150;

    hc = count = (ratio + 2) * 100;
};
